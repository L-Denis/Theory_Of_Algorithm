...
public class Algoritm {
    ...
    public Algoritm_Inf bubbleSort() {
        int[] array = Arrays.copyOf(_array, _array.length);
        Algoritm_Inf details = new Algoritm_Inf();
        int temp = 0;
        int substitutions = 0;
        int comparisons = 0;
    ...
    Algoritm_Inf heapify(int arr[], int n, int i, Algoritm_Inf details)
    {
        int largest = i;
        int l = 2 * i + 1;
        int r = 2 * i + 2;
    ...
    private void QuickSortInner(int[] array, int low, int high, Algoritm_Inf details)
    {
       details.setComparisons(details.getComparisons() + 1);
       if (array.length == 0)
            return;
